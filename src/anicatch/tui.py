...
from .downloader import HAS_LIBTORRENT, download_with_libtorrent, get_magnet_link
from .models import AnimeDetail, AnimeItem, SeasonInfo
...
class AnimeListItem(ListItem):
    ...
    def compose(self) -> ComposeResult:
        title = (
            self.anime.title[:50] + "..."
            if len(self.anime.title) > 50
            else self.anime.title
        )
        yield Label(
            f"[{self.index}] {title}    {self.anime.size}    {self.anime.publish_time}"
        )
...
class DetailView(Container):
    ...
    def _render_detail(self) -> str:
        ...
        lines = [
            ...
            "-" * 50,
            f"Magnet: {self.anime.magnet_link[:60]}..."
            if self.anime.magnet_link
            else "Magnet: 获取中...",
            "-" * 50,
            "操作: D下载 | Esc返回 | Q退出",
        ]
    ...
    async def load_season_animes(self) -> None:
        ...
        try:
            ...
            if season.months and self.current_month_index < len(season.months):
                self.query_one(SeasonHeader).month_name = season.months[
