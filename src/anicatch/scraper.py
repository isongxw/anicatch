...
def fetch_with_retry(
    url: str, max_retries: int = MAX_RETRIES, cookies: Optional[dict] = None
) -> Any:
    """
...
def search_anime(keyword: str) -> list[AnimeItem]:
    """
    在 miobt.com 上搜索动漫资源
    ...
    Returns:
        搜索结果列表
    """
    url = f"https://miobt.com/search.php?keyword={quote(keyword)}"
    try:
